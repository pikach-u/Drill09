...
class AutoRun:
    ...
    @staticmethod
    def enter(boy, e):
        if auto_run_down(e) or auto_run_up(e):
            if boy.action == 3:  # 오른쪽을 바라본 Idle 상태였다면
                boy.dir, boy.action = 1, 1
            elif boy.action == 2:  # 왼쪽을 바라본 Idle 상태였다면
                boy.dir, boy.action = -1, 0
    ...
    def do(boy):
        ...
        if get_time() - boy.start_time > 5:  # 5초가 지났다면 Idle
...
class StateMachine:
    def __init__(self, boy):
        ...
        self.cur_state = Idle
        self.table = {  # 상태 테이블
            Idle: {right_down: Run, left_down: Run, left_up: Run, right_up: Run, auto_run_down: AutoRun,
                   auto_run_up: AutoRun},
